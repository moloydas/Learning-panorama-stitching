...
def compute_homography_opencv(matches, feature_1, feature_2):
    ...
    h, mask = cv2.findHomography(matched_features_2, matched_features_1, method=cv2.RANSAC, ransacReprojThreshold=2)
...
if __name__ == '__main__':
    ...
    for i in range(len(feature_vec_list)):
        ...
        print(f'img size: {img_list[i].shape}')
        ...
        cv2.imshow('img_warped', merged_img)
        cv2.imshow('ref', ref)
        cv2.waitKey(0)
        cv2.destroyAllWindows()

        # #####################################################
        # ### Try 1: extending image and then warping
        # #####################################################
        # row = img_list[i].shape[0]
        # cols = img_list[i].shape[1]
        # print(f'img size: {img_list[i].shape}')
        ...
        # min_height = min(trans[1])
        # max_height = max(trans[1])
        # min_width = min(trans[0])
        # max_width = max(trans[0])

        # print(f'min: {min_width} {min_height}')
        # print(f'max: {max_width} {max_height}')
        ...
        # if min_width < 0:
        #     x_offset_left = int(-min_width)
        # if max_width > cols:
        #     x_offset_right = int(max_width - cols)
        # if min_height < 0:
        #     y_offset_top = int(-min_height)
        # if max_height > row:
        #     y_offset_bottom = int(max_height - row)
        ...
        # img_copy = img_list[int((i+1)%len(feature_vec_list))].copy()
        # img_copy = cv2.copyMakeBorder(img_copy, y_offset_top, y_offset_bottom, x_offset_left, x_offset_right, cv2.BORDER_CONSTANT)

        # ref_img = img_list[i].copy()
        # ref_img = cv2.copyMakeBorder(ref_img, y_offset_top, y_offset_bottom, x_offset_left, x_offset_right, cv2.BORDER_CONSTANT)

        # final_img_rows = img_copy.shape[0]
        # final_img_cols = img_copy.shape[1]

        # warped_img = cv2.warpPerspective(img_copy, H, (final_img_cols, final_img_rows))
        # merged_img = ref_img.copy()
        # merged_img[merged_img == 0] = warped_img[merged_img == 0]

        # cv2.imshow('img_1', img_copy)
        # cv2.imshow('img_1_warped', merged_img)
        # cv2.imshow('img_2', ref_img)
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
